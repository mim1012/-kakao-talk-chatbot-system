...
class TestGridCell:
    ...
    def test_cell_trigger(self):
        ...
        assert cell.last_triggered > 0  # last_triggered_time -> last_triggered
    ...
    def test_cell_cooldown(self):
        ...
        # 쿨다운 중에는 트리거 불가 - set_triggered는 상태를 바꿈
        assert cell.is_in_cooldown() is True
        
        # 쿨다운 후
        time.sleep(0.2)
        assert cell.is_in_cooldown() is False  # 이 메서드가 자동으로 상태 업데이트
        assert cell.status == CellStatus.IDLE
        assert cell.can_be_triggered() is True
    
    @pytest.mark.unit
    def test_cell_set_idle(self):
        """셀 idle 설정 테스트"""
        cell = GridCell(
            id="test",
            bounds=(0, 0, 100, 100),
        ...
        assert cell.detected_text_position is None

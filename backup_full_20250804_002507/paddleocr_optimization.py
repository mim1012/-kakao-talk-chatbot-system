...
# EasyOCR은 보조용으로만 사용 (fallback)
try:
    import easyocr
    EASYOCR_AVAILABLE = True
    print("✅ EasyOCR 보조 로드 성공")
except ImportError as e:
    print(f"⚠️ EasyOCR import 실패: {e}")
    easyocr = None
...
class MonitorManager:
    ...
    def _initialize_paddleocr_safe(self):
        """PaddleOCR 엔진 초기화 (극도로 안전한 설정으로 primitive 에러 완전 방지)"""
        try:
            if PADDLEOCR_AVAILABLE and PaddleOCR is not None:
                logger.info("🔄 PaddleOCR 엔진을 극도로 안전한 모드로 초기화합니다...")
                
                # 🛡️ 극도로 안전한 설정으로 primitive 에러 완전 방지
                import os
                ...
                os.environ['VECLIB_MAXIMUM_THREADS'] = '1'  # VecLib 스레드 제한
                os.environ['NUMEXPR_NUM_THREADS'] = '1'  # NumExpr 스레드 제한
                ...
                try:
                    ...
                    self.paddle_ocr = PaddleOCR(
                        lang='korean',  # PaddleOCR에서는 'korean' 사용
                        use_angle_cls=False,
                        show_log=False,
                        use_gpu=False,
                        enable_mkldnn=False,
                        cpu_threads=1,
                        det_limit_side_len=320,  # 더 작은 크기로 제한
                        det_limit_type='min',
                        rec_batch_num=1,
                        max_text_length=25,  # 텍스트 길이 제한
                        drop_score=0.5  # 낮은 점수 결과 필터링
                    )
                    logger.info("✅ PaddleOCR 1단계 초기화 완료")
                    
                except Exception as e1:
                    logger.warning(f"⚠️ 1단계 초기화 실패: {e1}")
                    ...
                    try:
                        ...
                        logger.info("✅ PaddleOCR 2단계 초기화 완료")
                        
                    except Exception as e2:
                        logger.warning(f"⚠️ 2단계 초기화 실패: {e2}")
                        ...
                        try:
                            ...
                            logger.info("✅ PaddleOCR 3단계 초기화 완료")
                            
                        except Exception as e3:
                            logger.error(f"❌ 모든 초기화 시도 실패: {e3}")
                            logger.info("🚫 PaddleOCR 초기화 포기 - OCR 기능 비활성화")
                            self.paddle_ocr = None
                            return
                ...
                if self.paddle_ocr:
                    try:
                        test_img = np.ones((50, 50, 3), dtype=np.uint8) * 255  # 작은 흰색 이미지
                        test_result = self.paddle_ocr.ocr(test_img, cls=False)
                        logger.info("✅ PaddleOCR 초기화 테스트 성공")
                    except Exception as test_e:
                        logger.warning(f"⚠️ PaddleOCR 테스트 실패: {test_e}")
                        logger.info("🚫 PaddleOCR 비활성화 - 안전을 위해 OCR 기능 중단")
                        self.paddle_ocr = None
            ...
            logger.info("🚫 OCR 기능 완전 비활성화")
            self.paddle_ocr = None
        
        # EasyOCR 보조 초기화
        if EASYOCR_AVAILABLE:
            try:
                logger.info("🔄 EasyOCR 보조 초기화 시작")
                self.easy_ocr = easyocr.Reader(['ko', 'en'], gpu=False, verbose=False)
                logger.info("✅ EasyOCR 보조 초기화 성공")
            except Exception as e:
                logger.error(f"❌ EasyOCR 보조 초기화 실패: {e}")
                self.easy_ocr = None

    def _capture_and_ocr_from_img(self, img: 'Image.Image') -> str:
        """
        이미지를 받아 OCR만 수행 (캡처는 하지 않음) - 안전한 처리
        Args:
        ...
        try:
            ...
            if img is None or img.size[0] == 0 or img.size[1] == 0:
                logger.debug("이미지가 유효하지 않습니다.")
                return ""
            
            # 🛡️ 이미지 크기 제한 (너무 크면 OCR 오류 발생 가능)
            if img.size[0] > 2000 or img.size[1] > 2000:
                # 큰 이미지는 리사이즈
                img = img.resize((min(img.size[0], 1920), min(img.size[1], 1080)), Image.Resampling.LANCZOS)
                
            # 이미지 전처리
            img = self._preprocess_image(img)
            
            # 🛡️ PaddleOCR 안전한 처리 (primitive 에러 방지)
            img_rgb = img.convert('RGB')
            img_array = np.array(img_rgb)
            
            # 배열 유효성 검사
            if img_array.size == 0:
                logger.debug("이미지 배열이 비어있습니다.")
                return ""
            ...
            if self.paddle_ocr:
                try:
                    ...
                    result = self.paddle_ocr.ocr(img_array, cls=False)
                    if result and result[0]:
                        all_text = ''.join([line[1][0] for line in result[0] if line[1] and line[1][0]])
                        return all_text
            ...
            if self.easy_ocr:
                try:
                    ...
                    logger.debug(f"EasyOCR fallback 처리 오류: {ocr_e}")
                    return ""
            else:
                logger.debug("PaddleOCR 엔진이 초기화되지 않았습니다.")
                
            return ""
            
        except Exception as e:
            logger.error(f"_capture_and_ocr_from_img 전체 오류: {e}")
            return ""

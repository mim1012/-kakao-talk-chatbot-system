            ...
            if PADDLEOCR_AVAILABLE and PaddleOCR is not None:
                ...
                try:
                    ...
                    self.paddle_ocr = PaddleOCR(
                        ...
                        show_log=False,
                        use_gpu=False,
                        enable_mkldnn=True,  # 성능 향상을 위해 활성화
                        cpu_threads=2,  # 2개 스레드로 늘림
                        det_limit_side_len=640,  # 640으로 증가
                        det_limit_type='min',
                        rec_batch_num=1,
                        max_text_length=50,  # 50자로 증가
                        drop_score=0.3  # 0.3으로 낮춰서 더 많이 감지
                    )
                    ...
                    try:
                        self.paddle_ocr = PaddleOCR(lang='korean', show_log=False, use_gpu=False)
